...
public class LoginUsuarioControlador {
	...
	public String mandarEmail(@RequestParam("username") String username, Model model) {
		try {
			...
			if (usuario != null) {
				String cadena_token;
				cadena_token = usuarioServicio.generarToken(usuario);
	...
	public String recuperarExitoso(@PathVariable String email, @PathVariable String token, Model model) {
		try {
			...
			if (usuario.getId_usuario() == tokenValido.getUsuario().getId_usuario()) {
			model.addAttribute("email", email);
			model.addAttribute("token", token);
			return "cambiarContrasena";
			}else {
				return  "redirect:/controller/ERRORPAGE?error=El+usuario+no+coincide.";
			}
	...
	public String confirmarCambioContrasena(@RequestParam("email") String email, @RequestParam("token") String token,
			@RequestParam("contrasenaNueva") String contrasenaNueva, Model model) {
		try {
			Usuario usuario = usuarioRepositorio.findFirstByEmailUsuario(email);
			Token tokenValido = tokenRepositorio.findByCadenaToken(token);
			if (usuario.getId_usuario() == tokenValido.getUsuario().getId_usuario()) {
